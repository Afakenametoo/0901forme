 ...
 * @Date: 2025-02-18 16:30:30
 * @LastEditTime: 2025-02-18 16:45:43
...
import '@/styles/components/measureIcon.css'
import pointMark from '@/assets/icons/label.png'
export default class MeatureTool {
    layerDistanceList = [] as any[]
    layerAreaList = [] as any[]
    setDistance = () => {}
    setArea = () => { }
     ...
     * @param {*} layerId
     */
    measureDistance(map:any,layerId:string) {
        this.layerDistanceList.push(layerId)
        var isMeasure = true
        map.doubleClickZoom.disable()
        let catchMark:any = null
        let isEdit = false
        let Geolist = [] as any[]
         ...
         * @param {*} _e
         */
        function addPointforJSON(_e:any) {
            if (isMeasure) {
                const point = {
                    type: 'Feature',
                    geometry: {
         ...
         * @param {*} jsonPoint
         */
        function drawLine(jsonPoint:any) {
         ...
         * @param {*} jsonPoint 点集
         */
        function addMeasureRes(jsonPoint:any) {
         ...
         * @param {*} coords 点坐标
         */
        function removePoint(coords:any) {
            if (jsonPoint.features.length > 0) {
         ...
         * @returns
         */
        function getLength(pointList:any) {
        ...
        map.on('click', function (_e:any) {
            addPointforJSON(_e)
        })

        map.on('mousemove', function (_e:any) {
            mouseMove(_e)
        })

        map.on('dblclick', function (_e:any) {
            finish(_e)
        })
        ...
        function createMarker() {
            ...
            return map.createMarker(markerParam)
        }
        ...
        // 计算点到直线最近距离的点
        function getNearestPointOnLine(list:any, moveCoord:any) {
        ...
        function getNearestDistance(point:any, point1:any, point2:any) {
            const P = {}
            const A = {}
            const B = {}
            P.x = point[0]
        ...
        function getNearestPoint(point:any, point1:any, point2:any) {
        ...
        // 屏幕距离
        function screenDistance(point1:any, point2:any) {
        ...
        // 计算编辑点在线段上的添加位置
        function getDragCoords(coords:any, list:any) {
            var x, y, x1, y1, x2, y2
            let index = 0
            x = coords[0]
            y = coords[1]
     ...
     * @param {*} layerId
     */
    measureArea(map:any,layerId:string) {
            ...
            if (jsonPoint.features.length > 0) {
                ...
                for (let i = 0; i < jsonPoint.features.length; i++) {
                    ...
                    if ((jsonPoint.features.length === i + 1) && !isMeasure) {
                        ...
                        element.innerHTML = getArea(pointList).toString()
                        //element.appendChild(edit)
                        element.appendChild(clear)
                        element.appendChild(close)
                        new mapboxgl.Marker(option).setLngLat(coords).addTo(map)
         ...
         * @returns
         */
        function getArea(pointList:any[]) {
